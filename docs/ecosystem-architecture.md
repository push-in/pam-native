...
| 2 | `pushinbr/pam-native-auth` | Sessions, OAuth/OIDC, passkeys, biometrics, secure token refresh | PHP + Kotlin + Swift | Provider applications and physical biometric devices |
...
| 5 | `pushinbr/pam-native-maps` | Native maps, markers, camera, geocoding and route overlays | PHP + Kotlin + Swift views | Maps API keys and physical-device rendering |
| 6 | `pushinbr/pam-native-media` | Picker, capture, metadata, transcoding and bounded media pipelines | PHP + Kotlin + Swift | Camera/photo-library devices |
| 7 | `pushinbr/pam-native-video` | Playback, tracks, PiP, DRM hooks and download hand-off | PHP + Kotlin + Swift views | DRM vendor credentials and physical devices |
...
| 9 | `pushinbr/pam-native-realtime` | WebSocket/SSE channels, reconnect, heartbeats and backpressure | PHP + Kotlin + Swift | Network fault and long-running soak tests |
...
| 11 | `pushinbr/pam-native-share-extension` | Incoming/outgoing shares and extension-to-app hand-off | PHP + Kotlin + Swift extension | Apple signing and inter-app device tests |
...
| 18 | `pushinbr/pam-native-scanner` | Barcode/document scanning and normalized results | PHP + Kotlin + Swift views | Camera devices and representative documents |
